...
path = getDirectory("Choose a location");
...
// * * * * * * * Section 04 - Preparing Images for Analysis * * * * * * * * * * * * * *				
		...
		run("Mean...", "radius=4");															//Massaging nuclei channel before thresholding - MEAN Filter 4pixel radius
...
// * * * * * * * Section 06 - Getting measurements from the Green Channel (CH1) * * * * * * * * * * * * * *				
			selectWindow("green");														//Selects the window with just Bio-sensor channel (green/ch1); see line 34
				...
				roiManager("select", i);														//Selects the ROI corresponding with the loop number
				roiManager("measure");														//Measures the ROI
				bsMean = getResult("Mean",i);														//Defines a variable for Bio-sensor Mean Intensity and collects the info from the results table
				bsMax = getResult("Max",i);														//Defines a variable for Bio-sensor Maximum Intensity and collects the info from the results table
				imageNum = (z+1);														//Defines a variable for the current image number
		    	nucNum = (i+1);														//Defines a variable for the current nuclei number
		    print(summaryFile,windowtitlenoext+","+imageNum+","+nucNum+","+nucArea+","+nucMean+","+bsMean+","+bsMax);								//Prints to summaryFile: filename, image#, #nuc, nuc#, nucArea, nucMeanInt, Ch1MeanInt, Ch1MaxInt
		  	}																			//Closing Analysis Loop for each nuclei

// * * * * * * * Section 07 - Saving & Closing Outputs * * * * * * * * * * * * * *				
s			selectWindow("mask1");
